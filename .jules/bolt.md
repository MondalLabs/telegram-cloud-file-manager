...
**Learning:** In MongoDB, querying by one field (`folder_id`) and sorting by another (`name`) without a covering compound index forces an in-memory blocking sort. This is a severe bottleneck for large directories as MongoDB limits in-memory sorts to 32MB.
